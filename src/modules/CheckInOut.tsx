...
import { MOCK_CHECKS, CHECK_STATUS_OPTIONS, SQUAD_ALIASES, USER_DIRECTORY } from '../constants';
import { 
  Eye, 
  MapPin, 
  ChevronRight, 
...
const StaticMap: React.FC<{ lat: number, lng: number }> = ({ lat, lng }) => (
  <div className="w-full h-40 bg-gray-100 border border-gray-200 rounded-lg relative overflow-hidden group">
    <div className="absolute inset-0 flex flex-col items-center justify-center">
      ...
      <span className="text-[10px] text-gray-500 mt-1 font-mono bg-white/50 px-2 rounded">{lat.toFixed(6)}, {lng.toFixed(6)}</span>
    </div>
...
export const CheckInOut: React.FC<CheckInOutProps> = ({ user }) => {
  ...
  const handleOpenNewCheck = () => {
    ...
    if (user?.rol === 'CUADRILLA') {
      const dbUser = USER_DIRECTORY.find(u => (u.email?.toLowerCase() || '') === (user.email?.toLowerCase() || ''));
  ...
  const calculateOvertime = (c: CheckRequest) => {
    if (!c.timeCheckOut) return 0;
    const end = new Date(c.timeCheckOut);
    ...
    if (end > threshold) {
      return (end.getTime() - threshold.getTime()) / (1000 * 60 * 60);
    }
    return 0;
  };
  ...
  return (
    <div className="flex h-full relative overflow-hidden">
          ...
          {groupedChecks.map(([date, items]) => (
            <div key={date}>
              <div className="bg-gray-100/50 px-4 py-2 text-[12px] font-bold text-gray-600 border-b border-gray-200 flex items-center gap-2">
                {date} <span className="bg-gray-200 text-gray-500 px-2 rounded-full text-[10px]">{items.length}</span>
              </div>
              {items.map(check => (
                <div 
                  key={check.id}
      ...
      {/* Detail Slider */}
      {selectedCheck && (
        <div className="absolute top-0 right-0 bottom-0 w-[400px] bg-white border-l border-gray-200 shadow-2xl z-20 flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-bold text-gray-400 uppercase">CheckIn Location</label>
              <StaticMap lat={selectedCheck.checkInLocation.lat} lng={selectedCheck.checkInLocation.lng} />
            </div>

            {selectedCheck.checkOutLocation && (
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-gray-400 uppercase">CheckOut Location</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Time Check In</label>
                <div className="text-sm text-gray-700">{new Date(selectedCheck.timeCheckIn).toLocaleString('es-ES')}</div>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Time Check Out</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Horas Registradas</label>
                <div className="text-sm font-bold text-gray-900">{calculateHours(selectedCheck).toFixed(2)} hrs</div>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Horas Extras Calc.</label>
                <div className="text-sm font-bold text-orange-600">{calculateOvertime(selectedCheck).toFixed(2)} hrs</div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                ...
                {isAdmin ? (
                  <select 
                    ...
                    onChange={(e) => handleUpdateCheck({...selectedCheck, status: e.target.value as CheckStatus})}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                  >
                    {CHECK_STATUS_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                ) : (
                   <div className="px-3 py-1 bg-gray-100 text-gray-600 text-sm inline-block rounded-full">{selectedCheck.status}</div>
                )}
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Horas Autorizadas <span className="text-blue-500">*</span></label>
                {isAdmin ? (
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => handleUpdateCheck({...selectedCheck, horasAutorizadas: Math.max(0, selectedCheck.horasAutorizadas - 0.5)})}
                    ...
                    <input 
                      type="number" 
                      step="0.01"
                      value={selectedCheck.horasAutorizadas}
                      onChange={(e) => handleUpdateCheck({...selectedCheck, horasAutorizadas: parseFloat(e.target.value) || 0})}
                      className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-center text-sm font-bold"
                    />
                    <button 
                      onClick={() => handleUpdateCheck({...selectedCheck, horasAutorizadas: selectedCheck.horasAutorizadas + 0.5})}
                      className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50"
                    >
                      <Plus size={16} />
                    </button>
                  ...
                  <div className="text-sm font-bold text-gray-900">{selectedCheck.horasAutorizadas.toFixed(2)} hrs</div>
                )}
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Comentarios</label>
                {isAdmin ? (
                  <textarea 
                    value={selectedCheck.comentarios}
                    onChange={(e) => handleUpdateCheck({...selectedCheck, comentarios: e.target.value})}
                    placeholder="Agregar comentarios..."
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm min-h-[100px] outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ) : (
                  <div className="text-sm text-gray-600 italic">{selectedCheck.comentarios || 'Sin comentarios.'}</div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">ID <span className="text-blue-400">*</span></label>
                <input 
                  type="text" 
                  readOnly 
                  value={formData.id} 
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Email <span className="text-blue-400">*</span></label>
                <input 
                  type="text" 
                  readOnly 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Cuadrilla <span className="text-blue-400">*</span></label>
                {isAdmin ? (
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase">CheckIn Location <span className="text-blue-400">*</span></label>
                <div className="p-4 border border-gray-100 rounded-xl bg-gray-50">
                   <div className="text-xs text-gray-500 mb-2 font-mono">19.444045, -99.204560</div>
                ...
                <div className="pt-6 border-t border-gray-100 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className="w-full border border-gray-200 rounded-lg px-4 py-2 text-sm min-h-[80px] outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
              )}
